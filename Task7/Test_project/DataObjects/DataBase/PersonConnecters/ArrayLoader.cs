...
namespace Test_project.DataBase.PersonConnecters
{
    class ArrayLoader
    {
        ...
        public string DeleteArray(object masterId)
        {
            MappedType master = GetMappedType(_masterType);
            MappedType composited = GetMappedType(_compositedType);

            string linkTableName = _schema.GetLinkTableName(master, composited);
            string query = string.Format("Delete from{0} where {1}= {2}", linkTableName, master.LinkName, masterId);
            return query;
        }
