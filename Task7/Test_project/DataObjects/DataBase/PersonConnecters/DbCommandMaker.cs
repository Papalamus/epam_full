...
namespace Test_project.DataBase.PersonConnecters
{
    class DbCommandMaker
    {
        ...
        public List<CustomizeCommandHandler> DeleteCommand(object ID)
        {
            var result = new List<CustomizeCommandHandler>();
            ...
            foreach (KeyValuePair<string, MemberInfo> keyVal in _mappedType.ArrayMembers)
            {
                var arrayMember = keyVal.Value;
                ArrayLoader ar = new ArrayLoader(arrayMember, _ormBase);
                var query = ar.DeleteArray(ID);
                result.Add(command => command.CommandText = query);    
            }
            return result;
        }
        ...
        public List<CustomizeCommandHandler> InsertCommand(object value)
        {
            ...
            foreach (var member in _mappedType.ArrayMembers)
            {
                ...
                Type compositedType = member.Value.GetReflectedType().GetElementType();

                DbCommandMaker elementCommand = _ormBase.GetConnectorMaker(compositedType);
                ...
                foreach (var obj in arrayList)
                {
                    if (obj==null)
                    {
                        //TODO Если объект в массиве null
                        continue;
                    }
